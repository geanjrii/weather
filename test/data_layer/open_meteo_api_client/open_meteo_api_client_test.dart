// ignore_for_file: prefer_const_constructors
import 'dart:convert';

import 'package:http/http.dart' as http;
import 'package:mocktail/mocktail.dart';
import 'package:test/test.dart';
import 'package:weather/data_layer/data_layer.dart';

class MockHttpClient extends Mock implements http.Client {}

class MockResponse extends Mock implements http.Response {}

class FakeUri extends Fake implements Uri {}

void main() {
  const mockLocationJson = {
    "results": [
      {
        "id": 4887398,
        "name": "Chicago",
        "latitude": 41.85003,
        "longitude": -87.65005
      }
    ]
  };
  const mockWeatherJson = {
    "latitude": 43,
    "longitude": -87.875,
    "generationtime_ms": 0.2510547637939453,
    "utc_offset_seconds": 0,
    "timezone": "GMT",
    "timezone_abbreviation": "GMT",
    "elevation": 189,
    "current_weather": {
      "temperature": 15.3,
      "windspeed": 25.8,
      "winddirection": 310,
      "weathercode": 63,
      "time": "2022-09-12T01:00"
    }
  };
  const mockQuery = 'mock-query';
  const mockLatitude = 41.85003;
  const mockLongitude = -87.6500;
  group('OpenMeteoApiClient', () {
    late http.Client httpClient;
    late OpenMeteoApiClient apiClient;

    setUpAll(() {
      registerFallbackValue(FakeUri());
    });

    setUp(() {
      httpClient = MockHttpClient();
      apiClient = OpenMeteoApiClient(httpClient: httpClient);
    });

    group('constructor', () {
      test('does not require an httpClient', () {
        expect(OpenMeteoApiClient(), isNotNull);
      });
    });

    group('locationSearch |', () {
      test('makes correct http request', () async {
        //arrange
        final response = MockResponse();
        when(() => response.statusCode).thenReturn(200);
        when(() => response.body).thenReturn('{}');
        when(() => httpClient.get(any())).thenAnswer((_) async => response);
        //act
        try {
          await apiClient.locationSearch(mockQuery);
        } catch (_) {}
        //assert
        verify(() => httpClient.get(Uri.https(
              'geocoding-api.open-meteo.com',
              '/v1/search',
              {'name': mockQuery, 'count': '1'},
            ))).called(1);
      });

      test('throws LocationRequestFailure on non-200 response', () async {
        // arrange
        final response = MockResponse();
        when(() => response.statusCode).thenReturn(400);
        when(() => httpClient.get(any())).thenAnswer((_) async => response);
        // act & assert
        expect(() async => apiClient.locationSearch(mockQuery),
            throwsA(isA<LocationRequestFailure>()));
      });

      test('throws LocationNotFoundFailure on error response', () async {
        // arrange
        final response = MockResponse();
        when(() => response.statusCode).thenReturn(200);
        when(() => response.body).thenReturn('{}');
        when(() => httpClient.get(any())).thenAnswer((_) async => response);
        // act & assert
        await expectLater(apiClient.locationSearch(mockQuery),
            throwsA(isA<LocationNotFoundFailure>()));
      });

      test('throws LocationNotFoundFailure on empty response', () async {
        // arrange
        final response = MockResponse();
        when(() => response.statusCode).thenReturn(200);
        when(() => response.body).thenReturn('{"results": []}');
        when(() => httpClient.get(any())).thenAnswer((_) async => response);
        // act & assert
        await expectLater(apiClient.locationSearch(mockQuery),
            throwsA(isA<LocationNotFoundFailure>()));
      });

      test('returns Location on valid response', () async {
        // arrange
        final response = MockResponse();
        final jsonLocation = jsonEncode(mockLocationJson);
        when(() => response.statusCode).thenReturn(200);
        when(() => response.body).thenReturn(jsonLocation);
        when(() => httpClient.get(any())).thenAnswer((_) async => response);
        // act
        final actual = await apiClient.locationSearch(mockQuery);
        // assert
        expect(
          actual,
          isA<Location>()
              .having((l) => l.name, 'name', 'Chicago')
              .having((l) => l.latitude, 'latitude', 41.85003)
              .having((l) => l.longitude, 'longitude', -87.65005),
        );
      });
    });

    group('getWeather |', () {
      test('makes correct http request', () async {
        // arrange
        final response = MockResponse();
        when(() => response.statusCode).thenReturn(200);
        when(() => response.body).thenReturn('{}');
        when(() => httpClient.get(any())).thenAnswer((_) async => response);
        // act
        try {
          await apiClient.getWeather(
              latitude: mockLatitude, longitude: mockLongitude);
        } catch (_) {}
        // assert
        verify(() =>
            httpClient.get(Uri.https('api.open-meteo.com', 'v1/forecast', {
              'latitude': '$mockLatitude',
              'longitude': '$mockLongitude',
              'current_weather': 'true',
            }))).called(1);
      });

      test('throws WeatherRequestFailure on non-200 response', () async {
        // arrange
        final response = MockResponse();
        when(() => response.statusCode).thenReturn(400);
        when(() => httpClient.get(any())).thenAnswer((_) async => response);
        // act & assert
        expect(
            () async => apiClient.getWeather(
                  latitude: mockLatitude,
                  longitude: mockLongitude,
                ),
            throwsA(isA<WeatherRequestFailure>()));
      });

      test('throws WeatherNotFoundFailure on empty response', () async {
        // arrange
        final response = MockResponse();
        when(() => response.statusCode).thenReturn(200);
        when(() => response.body).thenReturn('{}');
        when(() => httpClient.get(any())).thenAnswer((_) async => response);
        // act & assert
        expect(
            () async => apiClient.getWeather(
                  latitude: mockLatitude,
                  longitude: mockLongitude,
                ),
            throwsA(isA<WeatherNotFoundFailure>()));
      });

      test('returns weather on valid response', () async {
        // arrange
        final response = MockResponse();
        final jsonWeather = jsonEncode(mockWeatherJson);
        when(() => response.statusCode).thenReturn(200);
        when(() => response.body).thenReturn(jsonWeather);
        when(() => httpClient.get(any())).thenAnswer((_) async => response);
        final actual = await apiClient.getWeather(
          latitude: mockLatitude,
          longitude: mockLongitude,
        );
        expect(
          actual,
          isA<WeatherDataLayer>()
              .having((w) => w.temperature, 'temperature', 15.3)
              .having((w) => w.weatherCode, 'weatherCode', 63.0),
        );
      });
    });
  });
}
